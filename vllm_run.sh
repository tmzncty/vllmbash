...
# Added sudo back for running as non-root user with sudo privileges.
# Password likely needed only once due to sudo timeout.
sudo apt update
sudo apt install -y wget btop build-essential python3-pip
...
if [ -d "$MINICONDA_INSTALL_PATH" ]; then
    ...
    # Use -b for batch mode (no prompts) and -p to specify install location
...
python -m vllm.entrypoints.openai.api_server \
    ...
    --host $HOST_IP \
    --port $PORT \
    --uvicorn-log-level debug # Using debug log level
